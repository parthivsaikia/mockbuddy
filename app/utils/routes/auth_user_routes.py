...
# ... (signup_user_route remains the same) ...
...
def signup_user_route(
    ...
    # AuthService.create_user also hashes password
    new_user = auth_services.AuthService.create_user(db=db, user_in=user_in)
    return resp_m.UserResponse.from_orm(new_user)
...
async def login_for_access_token_user_route(
    ...
    return resp_m.TokenResponse(
        ...
        user=resp_m.UserResponse.from_orm(user)
    )
